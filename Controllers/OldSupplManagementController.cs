...
{
    public class OldSupplManagementController : Controller
    {
        private readonly ILogger<SupplyManagementController> _logger;
        public string connectionString;
        readonly IConfiguration _configuration;

        public OldSupplManagementController(ILogger<SupplyManagementController> logger, IConfiguration configuration)
        {
            _logger = logger;
        ...
        public IActionResult ShippedKits(object sender, EventArgs e)

        {

            XLWorkbook wb = new XLWorkbook();
            DataTable dt = CreateDataTable("SELECT [SITEID] as 'Site ID', [KitNumber] as 'Kit Number', KITTYPE As 'Type', KITSET As 'Shipment Key', [KITSTAT] as 'Kit Status',  REPLACE(UPPER(CONVERT(varchar, [SENDDATE], 106)), ' ', '/') AS 'Send Date', REPLACE(UPPER(CONVERT(varchar, [RECVDDATE], 106)), ' ', '/') AS 'Received Date', [ASSIGNED] As 'Assigned' FROM [BIL_IP_RANGE] WHERE (([SITEID] = '" + HttpContext.Session.GetString("sesCenter") + "'OR '" + HttpContext.Session.GetString("sesCenter") + "' = '(All)') AND ([SENDBY] IS NOT NULL) AND ([KITSTAT] = 'Shipped')) ORDER BY [SITEID], [KitNumber], [KITKEY]");
            ...
            using (var stream = new MemoryStream())
            {
                ...
                return File(
                    ...
                    "ShippedKits" + DateTime.Now.Year + ".xlsx");

            }
        }

        public IActionResult ReceivedKits(object sender, EventArgs e)

        {

            XLWorkbook wb = new XLWorkbook();
            DataTable dt = CreateDataTable("SELECT [SITEID] as 'Site ID', [KitNumber] as 'Kit Number', KITTYPE As 'Type', KITSET As 'Shipment Key', [KITSTAT] As 'Kit Status', REPLACE(UPPER(CONVERT(varchar, [SENDDATE], 106)), ' ', '/') AS 'Send Date',  REPLACE(UPPER(CONVERT(varchar, [RECVDDATE], 106)), ' ', '/') AS 'Recieved Date', [ASSIGNED] as 'Assigned' FROM [BIL_IP_RANGE] WHERE (([SITEID] = '" + HttpContext.Session.GetString("sesCenter") + "'OR '" + HttpContext.Session.GetString("sesCenter") + "' = '(All)') AND ([SENDBY] IS NOT NULL) AND (([KITSTAT] = 'Acceptable') OR ([KITSTAT] = 'Unacceptable'))) ORDER BY [SITEID], [KitNumber], [KITKEY]");
            wb.Worksheets.Add(dt, "ReceivedKits").Columns().AdjustToContents(); // easiest way to convert sql data to a excel doc
            ...
            using (var stream = new MemoryStream())
            {
                ...
                return File(
                    content,
                    "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                    "ReceivedKits" + DateTime.Now.Year + ".xlsx");
        ...
        // Supply Distribution sub module

        public IActionResult DamagedKits(object sender, EventArgs e)

        {

            XLWorkbook wb = new XLWorkbook();
            DataTable dt = CreateDataTable("SELECT [SITEID] as 'Site ID', [KitNumber] as 'Kit Number', [KITSTAT] as 'Kit Status', REPLACE(UPPER(CONVERT(varchar, [SENDDATE], 106)), ' ', '/') AS 'Send Date', REPLACE(UPPER(CONVERT(varchar, [RECVDDATE], 106)), ' ', '/') AS 'Damaged Date', [ASSIGNED] as 'Assigned',  [RECVDBY] As 'Damaged By', [SENDBY] as 'Send By' FROM [BIL_IP_RANGE] WHERE([SITEID] = '" + HttpContext.Session.GetString("sesCenter") + "'OR '" + HttpContext.Session.GetString("sesCenter") + "' = '(All)') AND([KITSTAT] = 'Unacceptable') ORDER BY[SITEID], SUBJID, [KitNumber], [KITKEY]");
            wb.Worksheets.Add(dt, "DamagedKits").Columns().AdjustToContents(); // easiest way to convert sql data to a excel doc
            ...
            using (var stream = new MemoryStream())
            {
                ...
                return File(
                    content,
                    "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                    "DamagedKits" + DateTime.Now.Year + ".xlsx");
        ...
        //public IActionResult ReceiptReporting(string KITKEY)
        //{
        ...
        //    string sql = "SELECT KITKEY, KitNumber, TreatmentGroup, SITEID FROM BIL_IP_RANGE WHERE KITKEY = @KITKEY";
        //    SqlCommand cmd = new SqlCommand(sql, conn);
        ...
        //        {
        //            selectCmd.Parameters.AddWithValue("@KITKEY", KITKEY);
        ...
        //                temp.TreatmentGroup = rdr["TreatmentGroup"].ToString();
        ...
        //}
        //[HttpPost]
        //public IActionResult ReceiptReporting(string SITEID, string KitNumber, string TreatmentGroup, string KITKEY, string RECVDDATE, string isExcrusion, string isRecieved, string SPAPDTC, string Isapproved, string reason, string username, string password)
        //{
        //    int SPKEY = int.Parse(HttpContext.Session.GetString("sesSPKey"));
        //    string userid = HttpContext.Session.GetString("suserid");
        ...
        //        {
        //      string rtn = IPReceive(KITKEY, RECVDDATE, isExcrusion, isRecieved, SPAPDTC, Isapproved, reason);
        //      if(rtn == "Unacceptable") {
        //            TempData["Message"] = "Kit Number " + KitNumber + " marked as Unacceptable condition ";
        //            string val = CheckAutoResupply(SPKEY, SITEID);
        //            if (val == "")
        //            {
        //                string result = ChkSiteInv(SPKEY, SITEID, TreatmentGroup);

        //            }
        //            return RedirectToAction("ReceiveIP");
        //        }
        //      else if (rtn == "acceptable")
        //        {
        //            TempData["Message"] = "Kit Number " + KitNumber + " marked as Received ";

        //        }
        //        else
        //        {
        //            TempData["ErrorMessage"] = rtn;
        //            return View();
        //        }
        //        string UserName = GetUserName(HttpContext.Session.GetString("suserid"));
        //        string subject = "Webview RTSM - Kit Processed at Site " + SITEID + "";

        //        string message = "Webview RTSM " + "\n" + "Kit " + KitNumber + " processed as received in " + rtn +" condition by " + UserName + " at Site " + SITEID + ".";
        ...
        public string IPReceive(string KITKEY, string RECVDDATE, string isExcrusion, string isRecieved, string SPAPDTC, string Isapproved, string reason)
        {
            string rtn = "";
            int SPKEY = int.Parse(HttpContext.Session.GetString("sesSPKey"));
            string userid = HttpContext.Session.GetString("suserid");
            ...
            if ((isExcrusion == "No") && (isRecieved == "Yes"))
            {
                sql = "UPDATE BIL_IP_RANGE SET  RECVDBY = @RECVDBY, RECVDDATE = sysdatetime(), KITSTAT = 'Acceptable', isExcrusion = @isExcrusion, isRecieved = 'Yes'  WHERE KITKEY = @KITKEY";
            }
            else if ((isExcrusion == "No") && (isRecieved == "No"))
            {
                sql = "UPDATE BIL_IP_RANGE SET  RECVDBY = @RECVDBY, RECVDDATE = sysdatetime(), KITSTAT = 'Unacceptable', ASSIGNED = 'Unacceptable', isExcrusion = @isExcrusion, isRecieved = 'No', reason = @reason  WHERE KITKEY = @KITKEY";
                recieve = "Unacceptable";
            }
            else if ((Isapproved == "Yes") && (isRecieved == "Yes"))
            {
                sql = "UPDATE BIL_IP_RANGE SET  RECVDBY = @RECVDBY, RECVDDATE = sysdatetime(), KITSTAT = 'Acceptable', isExcrusion = @isExcrusion, isRecieved = 'Yes', Isapproved = 'Yes', SPAPDTC = @SPAPDTC WHERE KITKEY = @KITKEY";
                recieve = "acceptable";
            }
            else if ((Isapproved == "Yes") && (isRecieved == "No"))
            {
                sql = "UPDATE BIL_IP_RANGE SET  RECVDBY = @RECVDBY, RECVDDATE = sysdatetime(), KITSTAT = 'Unacceptable', ASSIGNED = 'Unacceptable', isExcrusion = @isExcrusion, isRecieved = 'No', Isapproved = 'Yes', SPAPDTC = @SPAPDTC, reason = @reason WHERE KITKEY = @KITKEY";
                recieve = "Unacceptable";
            }
            else
            {
                sql = "UPDATE BIL_IP_RANGE SET  RECVDBY = @RECVDBY, RECVDDATE = sysdatetime(), KITSTAT = 'Unacceptable', ASSIGNED = 'Unacceptable', isExcrusion = @isExcrusion, isRecieved = 'No', Isapproved = 'No' WHERE KITKEY = @KITKEY";
                recieve = "Unacceptable";
            }
            connectionString = _configuration.GetConnectionString("VpeRandDbConnStr");
            SqlConnection conn = new System.Data.SqlClient.SqlConnection(connectionString);
            ...
            using (conn)
            {
                ...
                cmd.Parameters.AddWithValue("@KITKEY", KITKEY);
                //cmd.Parameters.AddWithValue("@KITSTAT", "Received");
                ...
                if ((SPAPDTC != null) && (Isapproved == "Yes"))
                {
                    cmd.Parameters.AddWithValue("@SPAPDTC", SPAPDTC);
                }
        ...
        public string ChkSiteInv(int spkey, string siteid, string arm)
        {
            ...
            {
                string sqlState;
                sqlState = "SELECT COUNT(*) AS CHKKITS FROM BIL_IP_RANGE WHERE (SITEID = '" + siteid + "') AND (TreatmentGroup = '" + arm + "') AND (ASSIGNED IS NULL) AND (RECVDBY IS NOT NULL)";
                ...
                if (cntArm <= 8)
                {
                    ...
                    else
                    {
                        ...
                        sqlState2 += "(SELECT TOP 8 KITKEY FROM BIL_IP_RANGE WHERE (TreatmentGroup = '" + arm + "') AND (SITEID IS NULL) ORDER BY KitNumber); ";
        ...
        [HttpPost]
        public IActionResult IPReceiptReporting(int KITSET, string SITEID, string SENDDATE, string SelectedKitKeys, IPReporting model, IFormFile PackageFile, IFormFile LogFile, string RECVDDATE, string isExcrusion, string SPAPDTC, string Isapproved, string username, string password)
        {
            int SPKEY = int.Parse(HttpContext.Session.GetString("sesSPKey"));
            string userid = HttpContext.Session.GetString("suserid");
            ViewBag.SiteID = SITEID;
            ...
            if (IsValidUser(username, password) && (string.Equals(userid, username, StringComparison.OrdinalIgnoreCase) || userid == "sidran"))
            {
                ...
                if (isExcrusion == "No")
                {
                    rtnVal = IPReceive2(SITEID, KITSET, "Acceptable", RECVDDATE, isExcrusion, SPAPDTC, Isapproved, username, PackageFile, LogFile);
                }
                else if (isExcrusion == "Yes" && Isapproved == "No")
                {
                    rtnVal = IPReceive2(SITEID, KITSET, "Unacceptable", RECVDDATE, isExcrusion, SPAPDTC, Isapproved, username, PackageFile, LogFile);
                }
                else if (isExcrusion == "Yes" && Isapproved == "Yes")
                {
                    if (SelectedKitKeys != null)
                    {
                        ...
                        {
                            rtnVal = UpdateRows(SITEID, SelectedKitKeys, KITSET, RECVDDATE, isExcrusion, SPAPDTC, Isapproved, username, CountKitSet(KITSET), PackageFile, LogFile);
                        }
                    }
                    else
                    {
                        rtnVal = IPProcessed(SITEID, KITSET, "Unacceptable", RECVDDATE, isExcrusion, SPAPDTC, Isapproved, username, PackageFile, LogFile);
                    }
                ...
                else if (rtnVal == "Kits has been processed" || rtnVal == "Unacceptable")
                {
                    if (rtnVal == "Unacceptable")
                        TempData["Message"] = "Shipment Key " + KITSET + " marked as Unacceptable condition ";
                    else
                        TempData["Message"] = rtnVal;

                }
                else if (rtnVal == "Acceptable")
                {
                    TempData["Message"] = "Shipment Key " + KITSET + " marked as received in acceptable condition ";
        ...
        public string IPReceive2(string SITEID, int KITSET, string proctype, string RECVDDATE, string isExcrusion, string SPAPDTC, string Isapproved, string username, IFormFile PackageFile, IFormFile LogFile)
        {
            string rtn = "";
            int SPKEY = int.Parse(HttpContext.Session.GetString("sesSPKey"));
            string userid = HttpContext.Session.GetString("suserid");
            ...
            {
                sql = "UPDATE BIL_IP_RANGE SET  RECVDBY = @RECVDBY, RECVDDATE = sysdatetime(), KITSTAT = '" + proctype + "', isExcrusion = @isExcrusion, SPAPDTC = @SPAPDTC  WHERE KITSET = @KITSET AND KITSTAT = 'Shipped'";
            }
            else if (proctype == "Acceptable")
            {
                sql = "UPDATE BIL_IP_RANGE SET RECVDBY = @RECVDBY, RECVDDATE = sysdatetime(), KITSTAT = '" + proctype + "', isExcrusion = @isExcrusion WHERE KITSET = @KITSET AND KITSTAT = 'Shipped'";
            }
            else if (proctype == "Unacceptable")
            {
                sql = "UPDATE BIL_IP_RANGE SET  RECVDBY = @RECVDBY, RECVDDATE = sysdatetime(), KITSTAT = '" + proctype + "', ASSIGNED = 'Unacceptable', isExcrusion = @isExcrusion WHERE KITSET = @KITSET AND KITSTAT = 'Shipped' ";
                //recieve = "Unacceptable";
            }
            else
            {
            ...
            else
            {
                ...
                if (proctype == "Unacceptable")
                {
                    ...
                    if (retSupp == "")
                    {
                        SendEmail(email + ";" + "sidran@amarexcro.com", subject, message);
                    }
                    else
                    {
                        SendEmail(email + ";" + retSupp, subject, message);
                    }
                    rtn = "Unacceptable";
                }
            }
        ...
        public string IPProcessed(string SITEID, int KITSET, string proctype, string RECVDDATE, string isExcrusion, string SPAPDTC, string Isapproved, string username, IFormFile PackageFile, IFormFile LogFile)
        {
            string rtn = "";
            int SPKEY = int.Parse(HttpContext.Session.GetString("sesSPKey"));
            string userid = HttpContext.Session.GetString("suserid");
            //string SITEID = HttpContext.Session.GetString("sesCenter");
            string sql = "";
            sql = "UPDATE BIL_IP_RANGE SET  RECVDBY = @RECVDBY, RECVDDATE = sysdatetime(), KITSTAT = '" + proctype + "', ASSIGNED = 'Unacceptable', isExcrusion = @isExcrusion, SPAPDTC = @SPAPDTC  WHERE KITSET = @KITSET AND KITSTAT = 'Shipped'";
        ...
        public string UpdateRows(string SITEID, string SelectedKitKeys, int KITSET, string RECVDDATE, string isExcrusion, string SPAPDTC, string Isapproved, string username, int totalkit, IFormFile PackageFile, IFormFile LogFile)
        {
            int SPKEY = int.Parse(HttpContext.Session.GetString("sesSPKey"));
            string userid = HttpContext.Session.GetString("suserid");
            //string SITEID = HttpContext.Session.GetString("sesCenter");
            ...
            {
                InsertReceived(Key[i], KITSET, RECVDDATE, isExcrusion, SPAPDTC, Isapproved, username, PackageFile, LogFile);
            }
            if (totalkit != Key.Count())
            {

                SqlConnection conn = new SqlConnection(_configuration.GetConnectionString("VpeRandDbConnStr"));
                string sql = "UPDATE BIL_IP_RANGE SET  RECVDBY = @RECVDBY, RECVDDATE = sysdatetime(), KITSTAT = 'Unacceptable', ASSIGNED = 'Unacceptable', isExcrusion = @isExcrusion  WHERE (KITSET = @KITSET) AND (KITSTAT = 'Shipped')";
                connectionString = _configuration.GetConnectionString("VpeRandDbConnStr");
                SqlCommand cmd = new SqlCommand(sql, conn);
            ...
            if (rowsAfft != 0)
            {
                ...
                message += "Kit Received in Unacceptable condition: " + rowsAfft + Environment.NewLine;
        ...
        public void InsertReceived(int Key, int KITSET, string RECVDDATE, string isExcrusion, string SPAPDTC, string Isapproved, string username, IFormFile PackageFile, IFormFile LogFile)
        {
            string userid = HttpContext.Session.GetString("suserid");
            SqlConnection conn = new SqlConnection(_configuration.GetConnectionString("VpeRandDbConnStr"));
            string sql = "UPDATE BIL_IP_RANGE SET RECVDBY = @RECVDBY, RECVDDATE = sysdatetime(), KITSTAT = 'Acceptable', isExcrusion = @isExcrusion WHERE KITKEY = @KITKEY AND KITSTAT = 'Shipped'";
            connectionString = _configuration.GetConnectionString("VpeRandDbConnStr");
            SqlCommand cmd = new SqlCommand(sql, conn);
            int rowsAfft;
            using (conn)
            ...
            if (PackageFile != null && LogFile != null)
            {
                try
                {
                    ...
                    if (allowedExtensions.Contains(fileExtension) && allowedExtensions.Contains(fileExtension2))
                    {
                        ...
                        {
                            TempData["Message"] = "Temperature Log and Packing List  uploaded succesfully.";
                            return RedirectToAction("ReceiveIPTabs");
                        }
                        else
                        {
                    ...
                    {
                        TempData["ErrorMessage"] = "Invalid file type. Only PDF files(.PDF) are permitted. Please ensure that you are uploading a file with the correct format.";
                    }
                }
                catch (Exception ex)
            ...
            else if (PackageFile != null)
            {
                try
                {
                    ...
                    if (allowedExtensions.Contains(fileExtension))
                    {
                        ...
                        if (rtnVal2 == "OK")
                        {
                            TempData["Message"] = "Packing List uploaded succesfully.";
                            return RedirectToAction("ReceiveIPTabs");
                        }
                        else
                        {
            ...
            else if (LogFile != null)
            {
                try
                {
                    ...
                    if (allowedExtensions.Contains(fileExtension2))
                    {
                        ...
                        if (rtnVal == "OK")
                        {
                            TempData["Message"] = "Temperature Log uploaded succesfully.";
                            return RedirectToAction("ReceiveIPTabs");
                        }
                        else
                        {
        ...
        public string ChkSiteInv2(int spkey, string siteid)
        {
            ...
            {
                string sqlState;
                sqlState = "SELECT COUNT(*) AS CHKKITS FROM BIL_IP_RANGE WHERE (SITEID = '" + siteid + "') AND (TreatmentGroup IS NOT NULL) AND (ASSIGNED IS NULL) AND (RECVDBY IS NOT NULL)";
                SqlConnection con = new SqlConnection(_configuration.GetConnectionString("VpeRandDbConnStr"));
                SqlCommand cmd = new SqlCommand(sqlState, con);
                con.Open();
                int cntArm = (int)cmd.ExecuteScalar();
                if (cntArm <= 8)
                {
                    ...
                    else
                    {
                        ...
                        string sqlState2 = "DECLARE @KS int; SELECT @KS = MAX(KITSET) FROM BIL_IP_RANGE; IF @KS is null SELECT @KS = 1 ELSE SELECT @KS = @KS + 1; ";
                        sqlState2 += "UPDATE BIL_IP_RANGE SET SITEID = '" + siteid + "', KITSET = @KS, SENDBY = 'SYSTEM', SENDDATE = '" + dteNow.ToString() + "', KITTYPE = 'Auto Re-supply', KITSTAT = 'Shipped' WHERE KITKEY IN ";
                        sqlState2 += "(SELECT TOP 8 KITKEY FROM BIL_IP_RANGE WHERE (TreatmentGroup IS NOT NULL) AND (SITEID IS NULL) ORDER BY KitNumber); ";
                        SqlCommand cmd3 = new SqlCommand(sqlState2, con);
                        int rowsAfft = (int)cmd3.ExecuteNonQuery();
                        if (rowsAfft <= 0)
                        {
...
//Previous Controllers for Kit level
//public IActionResult ReceiveIP()
//{
//    int SPKEY = int.Parse(HttpContext.Session.GetString("sesSPKey"));
//    string userid = HttpContext.Session.GetString("suserid");
//    string SITEID = HttpContext.Session.GetString("sesCenter");
//    connectionString = _configuration.GetConnectionString("VpeRandDbConnStr");
//    String sql = "SELECT [SITEID], [KitNumber], [KITSTAT], REPLACE(UPPER(CONVERT(varchar, [SENDDATE], 106)), ' ', '/') AS 'SEND DATE', REPLACE(UPPER(CONVERT(varchar, [RECVDDATE], 106)), ' ', '/') AS 'RECVD DATE', [ASSIGNED], [SUBJID], [KITKEY], [SENDBY], [TreatmentGroup] FROM [BIL_IP_RANGE] WHERE (([SITEID] = '" + HttpContext.Session.GetString("sesCenter") + "'OR '" + HttpContext.Session.GetString("sesCenter") + "' = '(All)') AND ([SENDBY] IS NOT NULL) AND ([KITSTAT] = 'Shipped') AND ([ATDEPOT] is not NULL)) ORDER BY [SITEID], SUBJID, [KitNumber], [KITKEY]";
//    String sql2 = "SELECT [SITEID], [KitNumber], [KITSTAT], REPLACE(UPPER(CONVERT(varchar, [SENDDATE], 106)), ' ', '/') AS 'SEND DATE', REPLACE(UPPER(CONVERT(varchar, [RECVDDATE], 106)), ' ', '/') AS 'RECVD DATE', [ASSIGNED], [SUBJID], [KITKEY], [RECVDBY], [SENDBY], [TreatmentGroup] FROM [BIL_IP_RANGE] WHERE ([SITEID] = '" + HttpContext.Session.GetString("sesCenter") + "'OR '" + HttpContext.Session.GetString("sesCenter") + "' = '(All)') AND ([SENDBY] IS NOT NULL) AND ([ASSIGNED] IS NULL) AND ([KITSTAT] = 'Acceptable') ORDER BY [SITEID], SUBJID, [KitNumber], [KITKEY]";
//    String sql3 = "SELECT [SITEID], [KitNumber], [KITSTAT], REPLACE(UPPER(CONVERT(varchar, [SENDDATE], 106)), ' ', '/') AS 'SEND DATE', REPLACE(UPPER(CONVERT(varchar, [RECVDDATE], 106)), ' ', '/') AS 'RECVD DATE', [ASSIGNED], [SUBJID], [KITKEY], [RECVDBY], [SENDBY], [TreatmentGroup] FROM [BIL_IP_RANGE] WHERE ([SITEID] = '" + HttpContext.Session.GetString("sesCenter") + "'OR '" + HttpContext.Session.GetString("sesCenter") + "' = '(All)') AND ([SENDBY] IS NOT NULL) AND ([KITSTAT] = 'Unacceptable') ORDER BY [SITEID], SUBJID, [KitNumber], [KITKEY]";
//    SqlConnection con = new System.Data.SqlClient.SqlConnection(connectionString);
//    SqlCommand cmd = new SqlCommand(sql, con);
//    SqlCommand cmd2 = new SqlCommand(sql2, con);
...
//            temp.SENDDATEstr = rdr["SEND DATE"].ToString();
//            temp.SENDBY = rdr["SENDBY"].ToString();
...
//        {
//            var temp = new ShipIP();
//            temp.KITKEY = (int)rdr3["KITKEY"];
//            temp.SITEID = rdr3["SITEID"].ToString();
//            temp.KitNumber = rdr3["KitNumber"].ToString();
//            temp.KITSTAT = rdr3["KITSTAT"].ToString();
//            temp.SUBJID = rdr3["SUBJID"].ToString();
//            temp.SENDDATEstr = rdr3["SEND DATE"].ToString();
//            temp.RECVDDATEstr = rdr3["RECVD DATE"].ToString();
//            temp.RECVDBY = rdr3["RECVDBY"].ToString();
//            temp.SENDBY = rdr3["SENDBY"].ToString();
//            temp.TreatmentGroup = rdr3["TreatmentGroup"].ToString();
//            shipip.Add(temp);
//        }
//        rdr3.Close();
//        SqlDataReader rdr4 = cmd3.ExecuteReader();
...
//public IActionResult UpdateAsRecieve(int KITKEY, string KitNumber)
//{

//    int SPKEY = int.Parse(HttpContext.Session.GetString("sesSPKey"));
//    string userid = HttpContext.Session.GetString("suserid");
//    string SITEID = HttpContext.Session.GetString("sesCenter");
//    string sql = "UPDATE BIL_IP_RANGE SET KITSTAT = @KITSTAT, RECVDBY = @RECVDBY, RECVDDATE = sysdatetime()  WHERE KITKEY = @KITKEY";
//    connectionString = _configuration.GetConnectionString("VpeRandDbConnStr");
//    SqlConnection conn = new System.Data.SqlClient.SqlConnection(connectionString);
//    SqlCommand cmd = new SqlCommand(sql, conn);
//    int rowsAfft;
...
//        conn.Open();
//        cmd.Parameters.AddWithValue("@KITKEY", KITKEY);
//        cmd.Parameters.AddWithValue("@KITSTAT", "Acceptable");
//        cmd.Parameters.AddWithValue("@RECVDBY", userid);
//        rowsAfft = (int)cmd.ExecuteNonQuery();
//    }
//    if (rowsAfft == 0)
...
//public IActionResult UpdateAsDamage(int KITKEY, string KitNumber, string TreatmentGroup)
//{

//    int SPKEY = int.Parse(HttpContext.Session.GetString("sesSPKey"));
//    string userid = HttpContext.Session.GetString("suserid");
//    string SITEID = HttpContext.Session.GetString("sesCenter");
//    string sql = "UPDATE BIL_IP_RANGE SET KITSTAT = @KITSTAT, RECVDBY = @RECVDBY, RECVDDATE = sysdatetime(), ASSIGNED = @ASSIGNED  WHERE KITKEY = @KITKEY ";
//    connectionString = _configuration.GetConnectionString("VpeRandDbConnStr");
//    SqlConnection conn = new System.Data.SqlClient.SqlConnection(connectionString);
//    SqlCommand cmd = new SqlCommand(sql, conn);
//    int rowsAfft;
...
//        conn.Open();
//        cmd.Parameters.AddWithValue("@KITKEY", KITKEY);
//        cmd.Parameters.AddWithValue("@KITSTAT", "Unacceptable");
//        cmd.Parameters.AddWithValue("@RECVDBY", userid);
//        cmd.Parameters.AddWithValue("@ASSIGNED", "Unacceptable");
//        rowsAfft = (int)cmd.ExecuteNonQuery();
//    }
//    if (rowsAfft == 0)
//    {
...
//        TempData["Message"] = "Kit Number " + KitNumber + " marked as damaged";
//        string UserName = GetUserName(HttpContext.Session.GetString("suserid"));
//        string subject = "Webview RTSM - Kit Processed at Site " + SITEID + "";
//        string message = "Webview RTSM " + "\n" + "Kit " + KitNumber + " processed as received in unacceptable condition by " + UserName + " at Site " + SITEID + ".";
//        string email = GetEmail(HttpContext.Session.GetString("suserid"));
//        SendEmail(email, subject, message);
//        string val = CheckAutoResupply(SPKEY, SITEID);
//        if (val == "")
